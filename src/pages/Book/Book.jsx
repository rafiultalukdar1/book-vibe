...
const Book = ({singleBook}) => {
    ...
    const {image, tags, bookName, author, category, rating } = singleBook;

    return (
        <>
            <div className='rounded-[16px] border border-[rgba(19,19,19,0.15)] p-[18px] flex flex-col justify-between gap-[16px]'>
                <div className='py-[32px] rounded-[16px] bg-[#F3F3F3]'>
                    <img className='w-[115px] h-[166px] object-cover mx-auto' src={image} alt="" />
                </div>
                <div className='flex items-center gap-[8px] pt-[8px]'>
                    {
                        tags.map(tag => <span className='text-[#23BE0A] text-[14px] font-[500] rounded-[30px] bg-[rgba(35,190,10,0.05)] py-[6px] px-[12px]'>{tag}</span>)
                    }
                </div>
